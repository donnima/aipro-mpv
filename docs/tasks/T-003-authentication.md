...
Unit:
10. The middleware public allow-list matches the documented list exactly — this test fails if someone adds a public route without updating the documentation.

E2E (Playwright, CI only):
11. Golden path: visit protected route → redirected → sign in via a test-mode magic link → land on the protected route.
